...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="AegisLift PPE 检测逐图 FP/FN 评估：IoU 匹配 TP/FP/FN 并输出 precision / recall。"
    )
    parser.add_argument("--weights", required=True, help="待评估的 best.pt 权重路径。")
    parser.add_argument("--data", required=True, help="dataset.yaml 路径。")
    parser.add_argument("--split", default="val", help="评估所用 split，默认 val。")
    parser.add_argument("--out-json", default=None, help="逐图 FP/FN JSON 输出路径。")
    parser.add_argument("--out-md", default=None, help="FP/FN Markdown 摘要输出路径。")
    parser.add_argument("--imgsz", type=int, default=640)
    parser.add_argument("--conf", type=float, default=0.25, help="预测保留置信度阈值。")
    parser.add_argument("--nms-iou", type=float, default=0.7, help="NMS 使用的 IoU 阈值。")
    parser.add_argument("--iou", type=float, default=0.5, help="GT 匹配使用的 IoU 阈值。")
...
def main() -> int:
    ...
    # 未显式指定输出路径时，默认写到权重同级目录，便于归档。
    default_out_dir = weight_path.parent
    output_json = (
        Path(args.out_json).expanduser().resolve()
        if args.out_json
        else default_out_dir / "ppe_fpfn_per_image.json"
    )
    output_md = (
        Path(args.out_md).expanduser().resolve()
        if args.out_md
        else default_out_dir / "ppe_fpfn_summary.md"
    )

    if not weight_path.exists():
        raise RuntimeError("权重文件不存在: {0}".format(weight_path))
    if not dataset_yaml.exists():
        raise RuntimeError("dataset.yaml 不存在: {0}".format(dataset_yaml))
    ...
    payload = {
        ...
        "matching": {"iou_threshold": args.iou},
        "totals": {
