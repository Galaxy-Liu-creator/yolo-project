...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="按 *.build.json 把多源 PPE 着装数据合并成一套带 split-manifest 的 YOLO 数据集。"
    )
    parser.add_argument(
        "--config",
        required=True,
...
def load_build_config(config_path: Path) -> Dict[str, object]:
    ...
    class_names = coerce_class_names(
        payload.get(
            "class_names",
            {
                0: "helmet",
...
def build_training_commands(
    *,
    ...
    # train.py 位于 train-code 根目录，data_tools 为其下一级子目录。
