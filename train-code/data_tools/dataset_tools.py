...
# 本文件位于 train-code/data_tools/ 子目录，而它依赖的 config.py 在上一级
# train-code/ 目录。这里把上一级目录注入 sys.path，保证 `import config`
# 在任意工作目录下都能稳定解析到 train-code/config.py。
...
def _prepare_personcrop_dataset(
    ...
    2. 再把原始 PPE 着装目标框分配给最合适的人框；
    3. 对每个人框裁出局部图像，并把 PPE 框映射到 crop 坐标系；
    4. 如果有 PPE 框无法归属到任何人框，可按配置回退为 fullframe 样本。
    """
    ...
    # 这里的人体检测模型只用于“切 person crop”，不参与最终 PPE 着装规范模型训练。
    ...
    for split_name, samples in split_map.items():
        ...
        for sample in samples:
            ...
            # 把每个 PPE 着装目标框分配给最合适的人框；分不出去的放进 unmatched。
            ...
            for person_index, person_box in enumerate(person_detections):
                ...
                if not assigned_boxes and not include_empty_person_crops:
                    # 如果该 person 没匹配到任何 PPE 框，且又不想保留空样本，就跳过。
                    continue
            ...
            if unmatched and fallback_to_fullframe:
                # 有些 PPE 框实在匹配不到任何 person 时，为了不丢正样本，
...
def assign_boxes_to_persons(
    ...
    """把每个 PPE 着装目标框分配给最合适的 person 检测框。"""
    ...
    for gt_box in gt_boxes:
        candidate_index = match_ppe_box_to_person(
            ppe_box=gt_box["bbox"],
...
def crop_person_sample(
    ...
    """裁剪单个人物区域，并把 PPE 着装目标框映射到 crop 局部坐标系。"""
...
def intersection_over_box(
    ...
    这里不用 IoU，而是用 IOA，是因为我们更关心“PPE 着装目标框有多少比例落在人框里”，
    而不是两个框整体是否相似。
    """
