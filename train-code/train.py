...
"""AegisLift 油田吊装作业 PPE/着装规范检测训练主入口脚本。
...
- `data_tools/dataset_tools.py` 负责数据工程逻辑；
- `config.py` 负责集中配置；
...
import config
from data_tools.dataset_tools import AuditResult, DatasetToolError, PrepareResult, audit_dataset, prepare_dataset
...
def bootstrap_project_config(argv: Optional[List[str]] = None) -> Optional[Path]:
    ...
    bootstrap_parser.add_argument(
        "--project-config",
        help="项目化 JSON 配置文件；默认尝试读取项目根目录下的 project_config.json。",
    )
...
def ensure_prepared_dataset(args) -> Tuple[AuditResult, PrepareResult]:
    ...
    prepare_payload = {
        ...
        "project_fit": {
            "pipeline_target": "person -> crop -> ppe",
...
def print_prepare_summary(prepare_result: PrepareResult) -> None:
    ...
    print("未匹配 PPE 框数    : {0}".format(prepare_result.unmatched_boxes))
...
def train_model(args) -> Dict[str, object]:
    ...
    requested_run_name = resolve_run_name(getattr(args, "name", None), "ppe")
...
def evaluate_model(args) -> Dict[str, object]:
    ...
    这里使用原生 YOLO `model.val()` 对 PPE/着装规范检测权重做指标评估。
    """
...
def export_model(args) -> Dict[str, object]:
    """导出最终 PPE/着装规范检测权重到 EXPORT_ROOT。"""
    ...
    export_target = export_root / "aegislift_ppe_yolov8.pt"
    ...
    metadata_path = export_root / "aegislift_ppe_yolov8.metadata.json"
    export_metadata = {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = StrictArgumentParser(
        description="AegisLift 油田吊装作业 PPE/着装规范检测 · YOLOv8 训练工具链",
    )
    ...
    train_parser = subparsers.add_parser("train", help="训练 PPE/着装规范检测器")
...
def add_prepare_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--person-model",
        help="用于 personcrop 数据准备的人体检测权重。",
    )
    parser.add_argument(
        "--person-conf",
        type=float,
    ...
    parser.add_argument(
        ...
        help="将 PPE/着装框匹配到 person 框的最小 IOA。",
    )
    parser.add_argument(
        "--monitored-person-labels",
        nargs="+",
    ...
    parser.add_argument(
        "--include-empty-person-crops",
        action="store_true",
        help="为未匹配 PPE/着装框的 person crop 生成空标注样本。",
    )
    parser.add_argument(
        "--no-fallback-to-fullframe",
        action="store_true",
        help="禁用 unmatched PPE/着装框的 fullframe 回退。",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
