...
# 当前配置文件所在目录，即 `train-code/`。
SCRIPT_ROOT = Path(__file__).resolve().parent
# 项目根目录（train-code 的上一级）。
PROJECT_ROOT = SCRIPT_ROOT.parent
# 仓库外原始数据默认与仓库同级放置在 `../datasets/raw`，也允许通过环境变量覆盖。
FRAME_LABEL_ROOT_ENV = "AEGISLIFT_DATA_ROOT"
...
def _resolve_external_data_root() -> Path:
    ...
    if not raw_value:
        return (PROJECT_ROOT.parent / "datasets" / "raw").resolve()
...
# 原始图片目录列表。
# 这里给出的是中性占位路径，实际路径请在 project_config.json 的 data.image_roots 中配置。
IMAGE_ROOTS = [
    _external_data_path("images"),
]

# 当前项目使用统一标签目录，标签文件通过“同名 stem”与图片配对。
# 这里给出的是中性占位路径，实际路径请在 project_config.json 的 data.label_root 中配置。
LABEL_ROOT = _external_data_path("labels")

# 支持识别为训练图片的扩展名。
IMAGE_EXTENSIONS = (".jpg", ".jpeg", ".png")

# 当前项目的 PPE 着装规范类别定义（顺序固定，类别 ID 不可变）。
CLASS_NAMES = {
    0: "helmet",     # 安全帽
    1: "no-helmet",  # 未佩戴安全帽
    2: "no-vest",    # 未穿反光衣
...
# personcrop 模式下，默认不保留“没有 PPE 标签”的空人物裁剪图。
...
# 将 PPE 框分配给 person 框时使用的最小 IOA 阈值。
DEFAULT_ASSIGNMENT_MIN_IOA = 0.35

# 当 PPE 框找不到匹配 person 时，是否允许回退为 fullframe 样本。
...
# 默认的人体检测模型候选路径列表。
# 优先尝试训练仓库自己的 weights 目录；如需扩展候选，请在 project_config.json 的
# models.person_model_candidates 中配置。
DEFAULT_PERSON_MODEL_CANDIDATES = [
    WEIGHTS_ROOT / "person_detect_yolov8.pt",
]

# 训练流程所使用的 Python 解释器候选。
# 默认直接使用当前解释器；如需指定其它解释器，
# 请在 project_config.json 的 models.python_candidates 中配置。
...
def resolve_sequence_name_from_image_root(image_root: Union[str, Path]) -> str:
    ...
    当原始图片位于各序列目录下的 `frames/` 子目录时，
    不能直接把最后一级目录名当成 sequence_name，需要回退到上一级目录名。
    """
...
def apply_project_config(
    ...
    *,
    ...
    - 未显式传入 `config_path` 时，默认尝试读取 `train-code/project_config.json`；
